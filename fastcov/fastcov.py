...
def plot(args, chunk, data) -> str:
    ...
    sns.lineplot(data=data, linewidth=0.8, dashes=False, alpha=1, legend=False, palette="gnuplot2")
    sns.set(style="dark")
    ...
    if args.logscale:
        ...
        plt.ylim(-yup / 50, yup + (yup / 50))
        plt.vlines([chunk.start + .1, chunk.end + .1], *plt.ylim(),
               'grey', linestyles='dashed', linewidth=1.2)
    ...
    short_names = [name.rsplit('/', 1)[-1][:-4] if name.endswith('.bam') else name.rsplit('/', 1)[-1]  
